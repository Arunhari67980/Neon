import { useRef, useState } from 'react';
import { useFrame } from '@react-three/fiber';
import { Sphere, MeshDistortMaterial, Html } from '@react-three/drei';
import * as THREE from 'three';
import { type Task, useStore } from '../../store/useStore';
import { Explosion } from './Explosion';

interface OrbitingTaskProps {
    task: Task;
    index: number;
    total: number;
}

export const OrbitingTask = ({ task, index, total }: OrbitingTaskProps) => {
    const meshRef = useRef<THREE.Mesh>(null!);
    const groupRef = useRef<THREE.Group>(null!);
    const [isExploding, setIsExploding] = useState(false);
    const { updateTask } = useStore();

    // Tasks orbit their planet
    const taskOrbitRadius = 2.5;
    const taskOrbitSpeed = 0.8;

    useFrame((state) => {
        if (!meshRef.current || isExploding) return;

        const time = state.clock.getElapsedTime();
        const angleOffset = (index / total) * Math.PI * 2;
        const angle = time * taskOrbitSpeed + angleOffset;

        meshRef.current.position.x = Math.cos(angle) * taskOrbitRadius;
        meshRef.current.position.z = Math.sin(angle) * taskOrbitRadius;
        meshRef.current.position.y = Math.sin(time * 2 + index) * 0.2; // Bobbing
        meshRef.current.rotation.y += 0.02;

        // Pulse effect for incomplete tasks
        if (!task.completed) {
            const scale = 1 + Math.sin(time * 3) * 0.1;
            meshRef.current.scale.set(scale, scale, scale);
        }
    });

    const handleComplete = (e: any) => {
        if (e.stopPropagation) e.stopPropagation();
        
        // Trigger explosion animation
        setIsExploding(true);
        
        // Complete the task after animation
        setTimeout(() => {
            updateTask(task.id, { completed: !task.completed });
            setIsExploding(false);
        }, 500);
    };

    if (isExploding) {
        return (
            <group ref={groupRef}>
                <Explosion />
            </group>
        );
    }

    return (
        <group ref={groupRef}>
            <Sphere
                ref={meshRef}
                args={[0.2, 32, 32]}
                onClick={handleComplete}
                position={[0, 0, 0]}
            >
                <MeshDistortMaterial
                    color={task.completed ? "#4ade80" : "#ffffff"}
                    speed={4}
                    distort={0.4}
                    emissive={task.completed ? "#4ade80" : "#ffffff"}
                    emissiveIntensity={task.completed ? 1 : 0.3}
                    roughness={0.3}
                    metalness={0.6}
                />

                <Html distanceFactor={10}>
                    <div
                        className={`
                            pointer-events-none select-none px-2 py-1 rounded-lg text-[8px] whitespace-nowrap
                            border transition-all duration-500 shadow-lg
                            ${task.completed
                                ? 'bg-green-500/20 border-green-500 text-green-300 shadow-glow-green'
                                : 'bg-white/10 border-white/30 text-white shadow-glow-white'
                            }
                        `}
                    >
                        {task.title}
                        {task.daily_streak > 0 && <span className="ml-1">🔥{task.daily_streak}</span>}
                    </div>
                </Html>
            </Sphere>

            {/* Small mini-orbit path for the task */}
            <mesh rotation={[-Math.PI / 2, 0, 0]}>
                <ringGeometry args={[taskOrbitRadius - 0.01, taskOrbitRadius + 0.01, 64]} />
                <meshBasicMaterial
                    color={task.completed ? "#4ade80" : "white"}
                    transparent
                    opacity={task.completed ? 0.3 : 0.05}
                />
            </mesh>
        </group>
    );
};
